...
const FMT_ERR_FIRST_CHARACTER_MUST_BE_A_PERCENT_SIGN: &str =
...
const FMT_ERR_SEXAGESIMAL_FRACTION_MUST_BE_3_5_6_8_OR_9: &str =
    "sexagesimal fraction must be 3, 5, 6, 8 or 9";
const FMT_ERR_SEXAGESIMAL_FORMAT_MUST_SPECIFY_WIDTH: &str = "sexagesimal format must spefify width";
const FMT_ERR_SEXAGESIMAL_FORMAT_MUST_SPECIFY_PRECISION: &str =
    "sexagesimal format must spefify precision";
...
pub fn format_sexagesimal(fmt: &NumberFormat, nbr: f64) -> String {
    match fmt.p {
        ...
        Some(6) => format!(
            "{}:{:02}:{:02.0}",
            nbr.trunc(),
            (nbr.fract() * 60f64).trunc().abs(),
            ((nbr.fract() * 60f64).fract() * 60f64).abs()
        ),
        Some(8) => format!(
            "{}:{:02}:{:04.01}",
            nbr.trunc(),
...
pub fn parse_sexagesimal(s: &str) -> Result<f64, ParseError> {
    ...
    match bytes.len() - j {
        ...
        6 => {
            if bytes[j + 3] as char == '.' {
                Ok(nbr + sign * parse_float(bytes, j + 1, bytes.len())? / 60f64)
            } else {
                Ok(nbr
                    + sign * parse_float(bytes, j + 1, j + 3)? / 60f64
                    + sign * parse_float(bytes, j + 4, bytes.len())? / 3600f64)
            }
        }
        8 | 9 => Ok(nbr
            + sign * parse_float(bytes, j + 1, j + 3)? / 60f64
        ...
        _ => Err(ParseError::new(
            FMT_ERR_SEXAGESIMAL_FRACTION_MUST_BE_3_5_6_8_OR_9,
            j,
        )),
    }
}
...
fn parse_float(bytes: &[u8], i: usize, j: usize) -> Result<f64, ParseError<'static>> {
    if i == j {
        return Err(ParseError::new(
            "begin and start indeces must not be equal",
            i,
        ));
    }
    if i > j {
        return Err(ParseError::new(
            "begin index must smaller than end index",
            i,
        ));
    }
...
impl NumberFormat {
    ...
    pub fn format_number(&self, n: impl Numeric) -> String {
        match self {
            NumberFormat {
                f: Some(FormatFlags(FormatFlags::ZERO)),
                w: Some(w),
                p: None,
                l: None,
                s: _,
            ...
            NumberFormat {
                f: Some(FormatFlags(FormatFlags::PLUS)),
                w: None,
                p: None,
                l: None,
                s: _,
    ...
    fn parse_width(
        ...
        if index == start {
            if let Some(f) = flags {
                if !f.zero_flag() {
                    return Err(ParseError::new(
                        FMT_ERR_SEXAGESIMAL_FORMAT_MUST_SPECIFY_WIDTH,
                        index,
                    ));
                }
            } else {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while index < value.len() {
            match value[index] as char {
                ...
                '\0' => {
                    return Err(ParseError::new(
                        FMT_ERR_PREMATURE_END_OF_FORMAT_STRING,
                        index,
                    ))
                }
                _ => break,
        ...
        let flags = if flags == UNSET_FORMAT_FLAGS {
            None
        } else {
            Some(flags)
        };
        ...
        while index < value.len() {
            match value[index] as char {
                ...
                '.' => {
                    if dot {
                        return Err(ParseError::new(
                            FMT_ERR_FORMAT_MUST_ONLY_CONTAIN_ONE_PERIOD,
                            index,
                        ));
                    }
        ...
        match specifier {
            ...
            'c' | 's' | 'p' | 'n' | '%' => {
                return Err(ParseError::new(FMT_ERR_UNSUPORTED_PRINTF_FORMAT, index))
            }
            '\0' => {
                return Err(ParseError::new(
                    FMT_ERR_PREMATURE_END_OF_FORMAT_STRING,
...
fn validate_sexagesimal(
    ...
    if let Some(p) = p {
        ...
        return Err(ParseError::new(
            FMT_ERR_SEXAGESIMAL_FORMAT_MUST_SPECIFY_PRECISION,
            i,
        ));
    }
    ...
    if let Some(w) = w {
        if w < p.unwrap() {
            return Err(ParseError::new(
                FMT_ERR_SEXAGESIMAL_WIDTH_MUST_EQUAL_OR_EXCEED_PRECISION,
                i,
            ));
        }
    } else {
...
mod tests {
    ...
    fn test_parse_sexagesimal() {
        ...
        assert_eq!(
            parse_sexagesimal("-10:30:18"),
            parse_sexagesimal("-10 30.3")
        );
        assert_eq!(
            parse_sexagesimal("-10:30:18.0"),
            parse_sexagesimal("-10 30.3")
        );
        assert_eq!(
            parse_sexagesimal("-10:30:18.00"),
            parse_sexagesimal("-10 30.3")
        );
        assert_eq!(
            parse_sexagesimal("-10:30:18"),
            parse_sexagesimal("-10;30.30")
        );
        assert_eq!(parse_sexagesimal("-10:30:18"), parse_sexagesimal("-10.505"));
        ...
        assert_eq!(
            format_sexagesimal(&"%9.6m".parse().unwrap(), nbr),
            "-10:30:18"
        );
    }
    ...
    fn test_number_format() {
        ...
        assert_format(
            "%0f",
            Some(FormatFlags(FormatFlags::ZERO)),
            None,
            None,
            None,
            'f',
        ...
        assert_format(
            "% 10i",
            Some(FormatFlags(FormatFlags::SPACE)),
            Some(10),
            None,
            None,
            'i',
        ...
        assert_format(
            "%-10d",
            Some(FormatFlags(FormatFlags::MINUS)),
            Some(10),
            None,
            None,
            'd',
        ...
        assert_format(
            "%+10x",
            Some(FormatFlags(FormatFlags::PLUS)),
            Some(10),
            None,
            None,
            'x',
        ...
        assert_format(
            "%#10X",
            Some(FormatFlags(FormatFlags::HASH)),
            Some(10),
            None,
            None,
            'X',
