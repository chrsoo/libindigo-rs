...
impl Client {
    ...
    pub fn attach<'a>(&mut self) -> Result<(), IndigoError> {
        let result = unsafe { indigo_attach_client(std::ptr::addr_of_mut!(self.sys)) };
    ...
    pub fn detach<'a>(&mut self) -> Result<(), IndigoError> {
        let result = unsafe { indigo_detach_client(std::ptr::addr_of_mut!(self.sys)) };
        map_indigo_result(result)
    }
}
